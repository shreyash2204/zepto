...
while True:
    ...
    try:
        ...
        print(f"Error reading input CSV or no more SKUs to process: {e}")
        break  # Exit the loop if no SKUs left or file not found

    # Process up to 20 SKUs in the current batch
    batch = skus_df.head(20)
    if batch.empty:
        print("No more SKUs to process.")
        break
    ...
    try:
        ...
        if not batch_df.empty:
            ...
            print(f"Batch of {len(batch)} SKUs saved to {output_csv}")
    except Exception as e:
        print(f"Error saving batch to output CSV: {e}")

    # Remove the processed SKUs from the input CSV
    try:
        skus_df = skus_df.iloc[20:]  # Drop the first 20 rows
        skus_df.to_csv(input_csv, index=False)
        print(f"Removed processed SKUs from {input_csv}")
    except Exception as e:
